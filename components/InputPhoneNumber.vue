...
<template>
  <div style="display: flex;" :class="[wrapperClasses, { disabled: disabled }]">
    <div
      v-if="enableFlagContainer"
    ...
    >
      <span class="vti__selection">
        ...
        <span v-if="enabledCountryCode" class="vti__country-code">+{{ activeCountry.dialCode }}</span>
    ...
    <div class="floating-label">
      <input
        ...
        :autofocus="autofocus"
        :class="['floating-input text-dark', inputClasses]"
        :disabled="disabled"
        :id="inputId"
        :maxlength="maxLen"
        :name="name"
      ...
      <label class="label-floater d-block d-md-none">{{parsedPlaceholderMobile}}</label>
      <label class="label-floater">{{parsedPlaceholder}}</label>
    </div>
  </div>
</template>
...
import utils, {
  getCountry,
  setCaretPosition
} from "~/plugins/input-phone-number/utils";
import clickOutside from "~/plugins/input-phone-number/click-outside";
...
export default {
  ...
  props: {
    ...
    enableFlagContainer: {
      type: Boolean,
      default: () => getDefault("enableFlagContainer")
    },
    ignoredCountries: {
      type: Array,
    ...
    placeholderMobile: {
      type: String,
      default: () => getDefault("placeholderMobile")
    },
    preferredCountries: {
      type: Array,
  ...
  watch: {
    ...
    phone(newValue, oldValue) {
      ...
      const isCustomValidate =
        this.customValidate && !this.testCustomValidate();
  ...
  async created() {
    ...
    if (this.enabledFlags) {
      await import("~/plugins/input-phone-number/sprite.css");
    }
  },
  methods: {
    initializeCountry() {
